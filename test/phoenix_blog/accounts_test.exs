defmodule PhoenixBlog.AccountsTest do
  ...
  alias PhoenixBlog.Accounts.{Admin, AdminToken}
  ...
  describe "get_admin_by_email_and_password/1" do
    test "does not return the admin if the email does not exist" do
  ...
  describe "apply_admin_email/3" do
    ...
    test "validates maximum value for e-mail for security", %{admin: admin} do
      too_long = String.duplicate("db", 100)
    ...
    test "validates e-mail uniqueness", %{admin: admin} do
      %{email: email} = admin_fixture()
    ...
    test "applies the e-mail without persisting it", %{admin: admin} do
      email = unique_admin_email()
  ...
  describe "update_admin_email/2" do
    ...
    test "updates the e-mail with a valid token", %{admin: admin, token: token, email: email} do
    ...
    test "does not update e-mail with invalid token", %{admin: admin} do
    ...
    test "does not update e-mail if admin e-mail changed", %{admin: admin, token: token} do
      assert Accounts.update_admin_email(%{admin | email: "current@example.com"}, token) == :error
      assert Repo.get!(Admin, admin.id).email == admin.email
      assert Repo.get_by(AdminToken, admin_id: admin.id)
    end

    test "does not update e-mail if token expired", %{admin: admin, token: token} do
  ...
  describe "confirm_admin/2" do
    ...
    test "confirms the e-mail with a valid token", %{admin: admin, token: token} do
    ...
    test "does not confirm e-mail if token expired", %{admin: admin, token: token} do
      {1, nil} = Repo.update_all(AdminToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
  ...
  describe "get_admin_by_reset_password_token/2" do
    setup do
      admin = admin_fixture()
  ...
  describe "reset_admin_password/3" do
    setup do
      %{admin: admin_fixture()}
    end
